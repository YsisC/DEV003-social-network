...
const db = oh(app);
const functionSignUp = (name2, email, password) => createUserWithEmailAndPassword(auth, email, password).then((result) => {
  if (name2 !== "") {
...
const functionSignin = (email, password) => signInWithEmailAndPassword(auth, email, password).then((userLogin) => userLogin.user);
const functionSignOut = () => signOut(auth);
const functionUserGoogle = () => {
  const provider = new GoogleAuthProvider();
  return signInWithPopup(auth, provider);
};
const currentUserInfo = () => auth.currentUser;
const saveTask = (tittle, description, displayName, uidCurrentUser) => {
  const today = new Date();
  return Ql(Ta(db, "tasks"), {
    uidCurrentUser,
    displayName,
    tittle,
    description,
...
const onGetTasks = (callback) => {
  const queryPost = sl(Ta(db, "tasks"), hl("date", "desc"));
  jl(queryPost, callback);
};
const deleteTask = (id2) => Gl(Aa(db, "tasks", id2));
const getTask = (id2) => Ol(Aa(db, "tasks", id2));
const updateTask = (id2, newFields) => Kl(Aa(db, "tasks", id2), newFields);
const addLikePost = (id2, uidCurrentUser) => {
  Kl(Aa(db, "tasks", id2), { like: of(uidCurrentUser) });
};
const removeLikePost = (id2, uidCurrentUser) => {
  Kl(Aa(db, "tasks", id2), { like: uf(uidCurrentUser) });
};
  ...
  buttonRegister.addEventListener("click", (e) => {
    ...
    functionSignUp(user, email, pasword).then((promiseResult) => {
      ...
      if (error.code === "auth/email-already-in-use") {
        ...
        Toastify({
          text: "La contrase\xF1a es muy debil",
          className: "info",
          style: {
            background: "linear-gradient(to right, #00b09b, #96c93d)"
          }
...
const Login = (onNavigate2) => {
  ...
  const feedFooter = document.createElement("footer");
  ...
  mensajeFeed.placeholder = `\xBFQue recetas estas pensando ${usuario}?`;
  buttonHome.className = "Cerrar_Sesion";
  buttonHome.textContent = "Cerrar Sesi\xF3n";
  feedPost.id = "taskDiv";
  labelReceta.setAttribute("id", "tittle");
  labelReceta.textContent = "\u25AA \xBFCual es el nombre de tu receta?:";
  ...
  onGetTasks((querySnapshot) => {
    ...
    querySnapshot.forEach((doc) => {
      ...
      html += `
        <div class='cardPostPublication'>
          ...
          <p class='displayName'> \u{1F468}\u{1F3FD}\u200D\u{1F373}${task.displayName}</p>
    ...
    btnsDelete.forEach((btn) => {
      btn.addEventListener("click", ({ target: { dataset } }) => {
        Swal.fire({
          title: "\xBFEsta seguro que desea eliminar esta publicacion?",
          icon: "warning",
